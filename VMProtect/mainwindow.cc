	...
	: QMainWindow(), temp_function_(NULL), fileChanged_(false)
{
	...
	home_page_act_->setToolTip("http://www.github.com");
...
#ifdef _MSC_VER
	toggle_ext_act_ = new QAction(QString::fromUtf8(language[lsToggle].c_str()), this);
	connect(toggle_ext_act_, SIGNAL(triggered()), this, SLOT(toggle_ext()));
#endif
...
#endif
#ifdef _MSC_VER
	tools_menu_->addAction(toggle_ext_act_);
#endif
...
#endif
#ifdef _MSC_VER
	toggle_ext_act_->setText(QString::fromUtf8(language[lsToggle].c_str()));
#endif
...
#ifdef _MSC_VER
bool IsDllRegistered() {
	wchar_t cls_id[MAX_PATH];
	StringFromGUID2(CLSID_VMPContextMenu, cls_id, _countof(cls_id));

	wchar_t sub_key[MAX_PATH];
	wsprintfW(sub_key, L"CLSID\\%s", cls_id);

	HKEY hKey;
	if (RegOpenKeyExW(HKEY_CLASSES_ROOT, sub_key, 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
		RegCloseKey(hKey);
		return true;
	}
...
BOOL IsRunningAsAdmin()
{
	...
	// Check whether the token has the admin group SID
...
void MainWindow::toggle_ext()
{
	if (!IsRunningAsAdmin()) {
		MessageDialog::warning(this, "You must run this program as administrator to perform this operation!", QMessageBox::Ok);
		return;
	}
	HMODULE hModule = LoadLibraryW(L"ShellExt.dll");
	if (hModule == NULL) {
		// GetLastError();
		MessageDialog::critical(this, "Cannot find proper extension DLL", QMessageBox::Ok);
		return;
	}
	...
	if (IsDllRegistered()) {
		...
		if (DllUnregisterServer == NULL) {
			MessageDialog::critical(this, "Can't locate function DllUnregisterServer() in extension DLL", QMessageBox::Ok);
		}
		else {
			HRESULT hr = DllUnregisterServer();
			if (SUCCEEDED(hr)) {
				MessageDialog::information(this, "DLL unregistered successfully!", QMessageBox::Ok);
			}
			else {
				MessageDialog::warning(this, QString("Failed to unregister DLL: 0x%1").arg(QString::number(hr, 16)), QMessageBox::Ok);
			}
		}
	}
	else {
		...
		if (DllRegisterServer == NULL) {
			...
			if (SUCCEEDED(hr)) {
				MessageDialog::information(this, "DLL registered successfully!", QMessageBox::Ok);
			}
			else {
				MessageDialog::warning(this, QString("Failed to register DLL: 0x%1").arg(QString::number(hr, 16)), QMessageBox::Ok);
			}
		}
	}
