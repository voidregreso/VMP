...
std::string VigenereEncrypt(const std::string& text, const std::string& key) {
	...
	for (size_t i = 0; i < text.size(); ++i) {
		char c = text[i];
		char keyC = key[i % key.size()];
		char encryptedChar = 'a' + (c - 'a' + keyC - 'a') % 26; // Assuming lowercase
...
void BaseFunction::AddWatermark(Watermark* watermark, int copy_count) {
	...
	std::string key = "1ff9936f1a2503f0805d9b284f8e2da9";  // Use a more secure key
	std::random_device rd;
	std::mt19937 generator(rd());
	std::uniform_int_distribution<unsigned int> distribution(0, 25);  // Random letter

	std::vector<uint8_t*> internal_watermarks = { version_watermark, owner_watermark };

	for (size_t k = 0; k < internal_watermarks.size(); k++) {
		if (k == 0) {
			if (!watermark)
				continue;
		}
			...
			std::string value(reinterpret_cast<char*>(ptr + 6), len);

			// Encrypt using Vigenère cipher
			value = VigenereEncrypt(value, key);

			// Add a random letter (salt) to the end of the encrypted value
			char salt = 'a' + static_cast<uint8_t>(distribution(generator));
			value += salt;
